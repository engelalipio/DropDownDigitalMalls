...
-(UIView*) tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    ...
    switch (section) {
        case 0:
            ...
            [header setText:[NSString stringWithFormat:@"%lu Total Categories",(unsigned long)mallCategories.count]];
            break;
...
-(UITableViewCell*) tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    airlineLogoUrlPng =  [Utilities getAzureMetaDataValue:mallCategories anyIndex:indexPath.row anyColumn:@"Name"];
    storeCount =         [Utilities getAzureMetaDataValue:mallCategories anyIndex:indexPath.row anyColumn:@"StoreCount"];
    ...
    if (airlineLogoUrlPng){
            ...
            image  = [Utilities getAzureStorageImage:mallCategories anyIndex:indexPath.row]; // [UIImage imageNamed:airlineLogoUrlPng];
   ...
   NSString *strToday = [dateFormatter stringFromDate:today];
    ...
    finalLocation = airlineLogoUrlPng;// [NSString stringWithFormat:@"Located Near %@",[Terminals objectAtIndex:terminalId] ];
...
- (void)viewDidLoad {
    ...
    if (appDelegate){
        mallCategories = [appDelegate  categoriesbackgrounds];
    }
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    ...
    if (selectedCell){
        ...
        catName = [Utilities getParseColumnValue:mallCategories anyIndex:self.tableView.indexPathForSelectedRow.row anyColumn:@"Name"];
        catLogo = [Utilities getParseColumnValue:mallCategories anyIndex:self.tableView.indexPathForSelectedRow.row anyColumn:@"CategoryName"];
    }
    
    ShopsViewController *fVC = (ShopsViewController *) [segue destinationViewController];
  
    if (fVC){
        [fVC setCategoryName:catLogo];
        [fVC setCatName:catName];
        [fVC setCatLogo:catLogo];
    }
